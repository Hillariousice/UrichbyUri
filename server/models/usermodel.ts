...
export const findById = async function(id: number){
    const data = await fetchData().find((info:IProducts) => info.id === id );
    return data          
}

export const createData = async function(details: IProductNew){
    const allData = await fetchData();
...
export const updateData = async function(update: IProductUpdate){
    const allData = await fetchData();
    const {productId, name, image, brand, category, description, price, countInStock, rating, numReviews} = await update;
    const index = allData.findIndex((data) => data.id == productId)
...
export const deleteData = async function(id: number){
    const allData = await fetchData();
    const index = allData.findIndex((info:IProducts) => info.id === id );
    if (index === -1){
        return -1;
    }
